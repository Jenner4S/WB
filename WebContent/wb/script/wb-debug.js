...
var Wb = {
    ...
    function query(item) {
      ...
      if (itemId && item.getValue && (!itemIds || Wb.indexOf(itemIds, itemId) != -1) &&
        ...
        if (item.getTextValue) { //获得显示值
          result['%' + itemId] = item.getTextValue();
        }
  ...
  /**
   * 生成指定url地址的图标HTML脚本。该脚本通常可用于表格、树、下拉框、HTML等。
   *
   * Example:
   *
   *     return Wb.getUrlIcon('wb/images/user.png', '用户') + value; //应用于Column的renderer配置项
   *
   * @param {String} url 图片文件的相对url地址。
   * @param {String} [title] 图标提示文本。
   * @return {String} HTML脚本。
   */
